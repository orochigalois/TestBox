...
extern int  Calibrate_X(unsigned  int ad_x,unsigned int ad_y);
extern int  Calibrate_X(unsigned  int ad_x,unsigned int ad_y);
...
int  AD2GUI_COORDX(unsigned  int ad_x,unsigned int ad_y)
{ 
        return Calibrate_X( ad_x , ad_y );
}
int  AD2GUI_COORDY(unsigned  int ad_x,unsigned int ad_y)
{
        return Calibrate_Y( ad_x , ad_y );
}
...
void GUI_TOUCH_Exec(void) {
 ...
 { 
    extern void GUI_TOUCH_Measure(void);
 ...
 //   #endif
	...
	{    
          ...
          __x = AD2GUI_COORDX(x,y);
          __y = AD2GUI_COORDY(x,y);
	
		//  if ( ( (__x <= 480) && (__x >= 0) )  && ( (__y <= 272) && (__y >= 0) ) )
		  {	  
		//     _StoreUnstable(-1, -1);
		  }
		//  else
		  {
            GUI_TOUCH_StoreUnstable(__x, __y);
		  }
	}
	else
	{
